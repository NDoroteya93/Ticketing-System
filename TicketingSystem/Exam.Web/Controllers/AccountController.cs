...
namespace TicketingSystem.Web.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        public async Task<ActionResult> Manage(string message)
        {
            ...
            var user = this.Data.ApplicationUsers.All().FirstOrDefault(x => x.Id == userId);
            ViewBag.StatusMessage = message ?? "";
            ViewBag.HasLocalPassword = await IdentityManager.Logins.HasLocalLoginAsync(User.Identity.GetUserId());
            ViewBag.ReturnUrl = Url.Action("Manage");
            ViewBag.UserRole = user.Roles.FirstOrDefault().Role.Name;
            return View();
        }

        public JsonResult GetUserTickets()
        {
            string userId = User.Identity.GetUserId();
            IQueryable<Ticket> listTickets = this.Data.Tickets.All().Where(x => x.Assignee.Id == userId);
            IQueryable<UserTicketsStatisticViewModel> statisticTicketList = listTickets
                 .GroupBy(x => new { x.TaskState })
                .Select(x => new UserTicketsStatisticViewModel
                {
                    TaskState = x.Key.TaskState,
                    Count = x.Count()
               })
               .OrderByDescending(x => x.Count);
            
            var jsonTicketState = JsonConvert.SerializeObject(statisticTicketList, Formatting.Indented, new JsonSerializerSettings { Converters = new[] { new StringEnumConverter() } });
            return this.Json(jsonTicketState, JsonRequestBehavior.AllowGet);
        }

        public ActionResult Read([DataSourceRequest] DataSourceRequest request)
        {
            string userId = User.Identity.GetUserId();
            var finishedTasks = this.Data.Tickets.All().Where(x =>x.TaskState == TaskState.Done && x.Assignee.Id == userId).Select(TicketBasicViewModel.FromTicket);
            return Json(finishedTasks.ToDataSourceResult(request));
        }
        //
        // POST: /Account/Manage
        [HttpPost]
